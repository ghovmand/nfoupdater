...
mdb.configuration(function(err, res) {
	baseimageurl = res.images.base_url + res.images.poster_sizes[1] ;
});
				...
				mdb.movieInfo({id: movie_id}, function(err, res) {
					...
					mdb.movieCredits({id: movie_id}, function(err, res) {
						...
						mdb.movieImages({id: movie_id}, function(err, res) {
							if(res.posters[0])
							{
								var imageurl = baseimageurl + res.posters[0].file_path;
								download(imageurl, path.join(dir, dirname, 'folder.jpg'), function() {
									console.log('Image dowloaded into ' + path.join(dir, dirname));
								});
							}
						});
					});
